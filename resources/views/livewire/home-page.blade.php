            ...
            <div class="grid grid-cols-1 gap-6 lg:grid-cols-4 md:grid-cols-2">
                @foreach ($brands as $brand)
                    <div class="bg-white rounded-lg shadow-md dark:bg-gray-800" wire:key="{{ $brand->id }}">
                        <p class="hidden">{{ $url_img = 'images/' . $brand->image }}</p>
                        <a href="/brands/{{ $brand->slug }}" class="">
                            <img src="{{ url($url_img) }}" alt="{{ $brand->name }}"
            ...
            <div class="grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-6">
                @foreach ($categories as $item)
                    <a class="group flex flex-col bg-white border shadow-sm rounded-xl hover:shadow-md transition dark:bg-slate-900 dark:border-gray-800 dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600"
                        wire:key="{{ $item->id }}" href="/categories/{{ $item->slug }}">
                        <div class="p-4 md:p-5">
                            <div class="flex justify-between items-center">
                                <div class="flex items-center">
                                    <p class="hidden">{{ $url_img = 'images/' . $item->image }}</p>

                                    <img class="h-[2.375rem] w-[2.375rem] rounded-full" src="{{ url($url_img) }}"
                                        alt="{{ $item->name }}">
                                    <div class="ms-3">
                                        <h3
